		...
		<TD colspan="2" class=label align=center>
			<TABLE cellspacing=0 cellpadding=2 width=200>
				  <TR>
							...
							<%else%>
									 <%if SearchOption = 1 then%>
									 <TD class=label align=center colspan=2><INPUT name=enviar type=button onClick="top.opener.document.forms[0].ConsignerData.value='<%=Name%>\n<%=Address%>\n<%if Address2 <> "" then response.Write Address2 & "\n" end if%><%=Phone1%>&nbsp;&nbsp;&nbsp;&nbsp;<%=Phone2%>';<%if Attn <> "" then response.Write "top.opener.document.forms[0].Attn.value='" & Name & "\nATTN:" & Attn & "';" end if%>top.opener.Consignee='<%=Name%>';top.opener.CountryConsignee='<%=Countries%>';top.opener.SetConsolidated();top.opener.document.forms[0].ConsignerID.value=<%=ObjectID%>;top.close();" value="&nbsp;&nbsp;Asignar&nbsp;&nbsp;" class=label></TD>
									 <%end if%>
